...
class Email:
    ...
    HTML messages.
    """
    def __init__(self, smtpServer, port):
        """
    ...
    def send(self):
        ...
        for fname,attachname in self._attach:
            if not os.path.exists(fname):
                print("O arquivo '%s' não existe.  Não foi anexado ao email." % fname)
                continue
            if not os.path.isfile(fname):
                print("O anexo '%s' não é um arquivo.  Não foi anexado ao email." % fname)
                continue
    ...
    def setSubject(self, subject):
        """
        Adiciono o assunto no email.
        """
        self._subject = subject
    
    def setFrom(self, address):
        """
        Atrelo o endereço de envio.
        """
        if not self.validateEmailAddress(address):
            raise Exception("Endereço de email inválido: '%s'" % address)
        self._from = address
    
    def clearRecipients(self):
        """
        Remove todos os recipientes do email.
        """
        self._to = []
    
    def addRecipient(self, address):
        """
        Adiciono mais um recipiente no email.
        """
        if not self.validateEmailAddress(address):
    ...
    def setTextBody(self, body):
        """
        Deixo a mensagem como 'plain text'.
        """
        self._textBody = body
...
if __name__ == "__main__":
    ...
    """
    mFrom = 'alertas@gruporecovery.com'
    mTo = ['fbarontini@gruporecovery.com','vnobrega@gruporecovery.com','jpinheiro@gruporecovery.com']
    
    e = Email('192.168.32.74',25)
    e.setFrom(mFrom)
    [e.addRecipient(recipient) for recipient in mTo];
    
    
    e.setSubject('Batimento de Carteira Ajuizada')
    e.setTextBody(None)
    e.setHtmlBody("Teste para envio do <b>batimento</b>")
