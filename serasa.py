...
def infere_cpf(cpf):
	...
	if len(cpf) == 8:
		...
		raiz = np.array([int(i) for i in cpf])
		...
		print('Digite um cpf válido, com as 9 primeiras posições')
...
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
def get_participacao_detalhes(s):
	# trago a informação se a empresa possui participacao
	# em outras empresas, com a relação de cnpj e nome de
	# cada uma, assim como a situação da empresas
	target = '#L010113'
	i = s.find(target)
	...
	while i != -1:
		...
		dic['cnpj'] = s[i+a:i+b] #infere_cnpj()
		dic['digito'] = s[i+b:i+c]
		dic['empresa_ligada'] = s[i+c:i+d].strip()
		dic['restricao'] = s[i+d:i+e].strip()
		dic['seq_serasa'] = s[i+e:i+f].strip()
		dic['tipo_pessoa'] = s[i+f:i+g].strip()
		dic['situacao'] = s[i+g:i+h].strip()
		
		if dic['tipo_pessoa'] == 'J':
			dic['cnpj'] = infere_cnpj(dic['cnpj'][1:])
		
		# TODO: ajustar o socio/avalista
...
def le_excel(arquivo):
	...
	writer = pd.ExcelWriter('retorno.xlsx')
	df['faturamento'] = df['StringConsulta'].apply(get_faturamento_presumido)
	df['score']=df['StringConsulta'].apply(get_pontuacao)
	df.to_excel(excel_writer=writer, header=True, index=False, sheet_name='Result')
	writer.save()
